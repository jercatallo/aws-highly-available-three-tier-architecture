...
export interface StackConfiguration {
  ...
  resourceNames: {
    ...
    webTierSecurityGroup: string;
    appTierSecurityGroup: string;
    databaseSecurityGroup: string;
    publicNetworkAcl: string;
    privateNetworkAcl: string;
    applicationLoadBalancer: string;
    webTierTargetGroup: string;
    httpListener: string;
    webTierAutoScalingGroup: string;
    webTierLaunchTemplate: string;
    webTierCpuScalingPolicy: string;
    appTierAutoScalingGroup: string;
    appTierLaunchTemplate: string;
    appTierCpuScalingPolicy: string;
    database: string;
  ...
  outputs: {
    ...
    enabled: {
      ...
      webTierAutoScalingGroupName: boolean;
      webTierAutoScalingGroupArn: boolean;
      appTierAutoScalingGroupName: boolean;
      appTierAutoScalingGroupArn: boolean;
...
export interface ProcessedStackConfig extends Omit<StackConfiguration, 'compute' | 'database'> {
  ...
  compute: {
    ...
    webTier: Omit<StackConfiguration['compute']['webTier'], 'instanceClass' | 'instanceSize' | 'machineImageType'> & {
      instanceClass: ec2.InstanceClass;
      instanceSize: ec2.InstanceSize;
      machineImage: ec2.IMachineImage;
    };
    appTier: Omit<StackConfiguration['compute']['appTier'], 'instanceClass' | 'instanceSize' | 'machineImageType'> & {
      instanceClass: ec2.InstanceClass;
      instanceSize: ec2.InstanceSize;
      machineImage: ec2.IMachineImage;
    };
...
function processConfiguration(config: StackConfiguration): ProcessedStackConfig {
  ...
  return {
    ...
    compute: {
      ...
      webTier: {
        ...config.compute.webTier,
        instanceClass: ec2.InstanceClass[config.compute.webTier.instanceClass as keyof typeof ec2.InstanceClass],
        instanceSize: ec2.InstanceSize[config.compute.webTier.instanceSize as keyof typeof ec2.InstanceSize],
        machineImage: config.compute.webTier.machineImageType === 'AMAZON_LINUX_2023'
          ? ec2.MachineImage.latestAmazonLinux2023()
          : ec2.MachineImage.latestAmazonLinux2(),
      },
      appTier: {
        ...config.compute.appTier,
        instanceClass: ec2.InstanceClass[config.compute.appTier.instanceClass as keyof typeof ec2.InstanceClass],
        instanceSize: ec2.InstanceSize[config.compute.appTier.instanceSize as keyof typeof ec2.InstanceSize],
        machineImage: config.compute.appTier.machineImageType === 'AMAZON_LINUX_2023'
          ? ec2.MachineImage.latestAmazonLinux2023()
          : ec2.MachineImage.latestAmazonLinux2(),
      },
      storage: {
 ...
 * export ENVIRONMENT=staging    # Loads staging.json
