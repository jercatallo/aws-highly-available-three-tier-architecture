...
describe('Outputs Construct', () => {
  ...
  let webTierAsg: autoscaling.AutoScalingGroup;
  let appTierAsg: autoscaling.AutoScalingGroup;
  ...
  beforeEach(() => {
    ...
    webTierAsg = new autoscaling.AutoScalingGroup(stack, 'TestWebTierAsg', {
      vpc,
      instanceType: ec2.InstanceType.of(ec2.InstanceClass.T2, ec2.InstanceSize.MICRO),
      machineImage: ec2.MachineImage.latestAmazonLinux2023(),
    });

    appTierAsg = new autoscaling.AutoScalingGroup(stack, 'TestAppTierAsg', {
      vpc,
      instanceType: ec2.InstanceType.of(ec2.InstanceClass.T2, ec2.InstanceSize.MICRO),
      machineImage: ec2.MachineImage.latestAmazonLinux2023(),
    });
  ...
  test('creates CloudFormation outputs for 3-tier architecture', () => {
    new OutputsConstruct(stack, 'TestOutputs', {
      vpc,
      loadBalancer: alb,
      webTierAutoScalingGroup: webTierAsg,
      appTierAutoScalingGroup: appTierAsg,
      database,
      environment: 'test',
    });
    ...
    expect(outputKeys.some(key => outputs[key].Description === 'Web Tier Auto Scaling Group Name')).toBe(true);
    expect(outputKeys.some(key => outputs[key].Description === 'Application Tier Auto Scaling Group Name')).toBe(true);
  ...
  test('exports VPC information', () => {
    ...
    expect(outputs[vpcIdOutput!].Export.Name).toBe('test-HighlyAvailable3Tier-VpcId');
    expect(outputs[vpcCidrOutput!].Export.Name).toBe('test-HighlyAvailable3Tier-VpcCidr');
  });
  ...
  test('exports load balancer DNS', () => {
    ...
    expect(outputs[loadBalancerDnsOutput!].Export.Name).toBe('prod-HighlyAvailable3Tier-LoadBalancerDNS');
  });
});
