...
export interface WebTierAsgConstructProps {
  vpc: ec2.Vpc;
...
/**
 * Web Tier Auto Scaling Group Construct for 3-Tier Architecture
 * 
 * Creates an Auto Scaling Group for web tier servers (presentation layer)
 * 
 * The Web Tier:
 * - Receives traffic from Application Load Balancer
 * - Deploys instances across multiple availability zones in private subnets
 * - Handles HTTP requests and serves static content
 * - Forwards business logic requests to the Application Tier
 ...
 */
export class WebTierAsgConstruct extends Construct {
  public readonly autoScalingGroup: autoscaling.AutoScalingGroup;

  constructor(scope: Construct, id: string, props: WebTierAsgConstructProps) {
    super(scope, id);
    ...
    this.autoScalingGroup = new autoscaling.AutoScalingGroup(this, props.autoScalingGroupName || 'WebTierAutoScalingGroup', {
      vpc: props.vpc,
